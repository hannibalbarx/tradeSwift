...
def data(path, col1, col2, label_path=None):
    for t, line in enumerate(open(path)):
        ...
        tw = 145
	tw += 1
	x[tw] = abs(hash(str(tw)+"_"+row[col1]+"_x_"+row[col2])) % D
...
start = datetime.now()
for col1 in range(145):
	for col2 in range(col1+1, 146):
		...
		ID2=0
		for ID, x, y in data(train, col1, col2, label):
		    ID2+=1
		    for k in K:
			p = predict(x, w[k])
